...
class Validator {
	...
	private function max(int $value) {
		if (strlen($this->arrayForValidation[$this->currentField]) > $value) {
			...
			return $this->addError("Поле должно быть не больше $value символов");
		}
	}

	private function min(int $value) {
		if (strlen($this->arrayForValidation[$this->currentField]) < $value) {
			return $this->addError("Поле должно быть не меньше $value символов");
		}
	}
	...
	private function filter_nospace() {
		if (preg_match("/\\s/", $this->arrayForValidation[$this->currentField])) {
			...
			return $this->addError("{$field} не должно содержать пробелы");
		}
	}

	private function filter_birthday() {
		$birthday = strtotime(date($this->arrayForValidation[$this->currentField]));
		...
		$eightYearsAgo = strtotime( "-8 years" );

		if (!checkdate( $mm, $dd, $yy ) || $hundredYearsAgo > $birthday || $birthday > $eightYearsAgo) {
			return $this->addError("Укажите настоящую дату рождения");
		}
	}
